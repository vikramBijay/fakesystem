    ...
    <div style={{
      ...
      <button
        ...
        onMouseLeave={e => {
          ...
          e.currentTarget.style.color = '#9d97c0';
          e.currentTarget.style.borderColor = 'rgba(255,255,255,0.12)';
        }}
      >✕</button>

      {/* ── Header ── */}
      <div style={{ textAlign: 'center', marginBottom: '2.8rem', animation: 'slideUp .45s ease both' }}>
        <div style={{
          display: 'inline-flex',
          alignItems: 'center',
          gap: '7px',
...
function PlanCard({ plan, billing, onClose }) {
  ...
  // ── Price display logic ──────────────────────────────────────────────
  // Use a single wrapper with font-variant-numeric: tabular-nums so
  // every digit takes the same width and $ never looks mismatched.
  // "From" prefix stays at the same size as the dollar sign.
  const renderPrice = () => {
    return (
      <span style={priceNumStyle}>
        ${price}
      </span>
    );
  };
  ...
  return (
    <div style={{
      ...
      boxShadow: isHighlighted
        ? '0 0 70px rgba(109,40,217,0.22), 0 0 0 1px rgba(139,92,246,0.15)'
        : 'none',
    ...
    onMouseEnter={e => {
      ...
      e.currentTarget.style.boxShadow = isHighlighted
        ? '0 0 90px rgba(109,40,217,0.38), 0 0 0 1px rgba(139,92,246,0.3)'
        : '0 0 40px rgba(109,40,217,0.14)';
    }}
    onMouseLeave={e => {
    ...
    >
      ...
      <div style={{
        ...
        height: '44px',
        borderRadius: '12px',
        background: isHighlighted ? 'rgba(139,92,246,0.25)' : 'rgba(139,92,246,0.08)',
        border: `1px solid ${isHighlighted ? 'rgba(139,92,246,0.5)' : 'rgba(139,92,246,0.15)'}`,
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        marginBottom: '1.2rem',
        boxShadow: isHighlighted ? '0 0 18px rgba(109,40,217,0.35)' : 'none',
      }}>
      ...
      {/* ── Price block ───────────────────────────────────────────────────
          Fixed height container so all three cards align perfectly.
          The number uses font-variant-numeric tabular-nums + a single
          font-size so $ and digits are always the same size.
      ──────────────────────────────────────────────────────────────────── */}
      <div style={{
        marginBottom: '1.8rem',
        minHeight: '64px',
        ...
        {renderPrice()}
        {price !== 0 && (
          <span style={priceMetaStyle}>
            USD / mo<br />
            <span style={{ color: '#4a4670' }}>
      ...
      <button
        ...
        style={{
          ...
          ...(plan.ctaStyle === 'primary' ? {
            ...
            color: '#fff',
            border: 'none',
            boxShadow: '0 0 26px rgba(109,40,217,0.55)',
          } : {
        ...
        onMouseEnter={e => {
          if (plan.ctaStyle === 'primary') {
            e.currentTarget.style.boxShadow = '0 0 44px rgba(109,40,217,0.75)';
        ...
        onMouseLeave={e => {
          if (plan.ctaStyle === 'primary') {
            e.currentTarget.style.boxShadow = '0 0 26px rgba(109,40,217,0.55)';
            e.currentTarget.style.transform = '';
          } else {
        ...
        {plan.features.map(feat => (
          <div key={feat} style={{
            ...
            <span style={{
              ...
              height: '17px',
              borderRadius: '50%',
              background: isHighlighted ? 'rgba(139,92,246,0.25)' : 'rgba(139,92,246,0.1)',
              border: `1px solid ${isHighlighted ? 'rgba(139,92,246,0.5)' : 'rgba(139,92,246,0.2)'}`,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              flexShrink: 0,
...
// ── Shared price typography styles ────────────────────────────────────────
// Identical across all three cards — $ and digits at same size/weight.
// tabular-nums keeps digit widths equal so numbers never shift layout.
const priceNumStyle = {
  fontFamily: "'Syne', sans-serif",
  fontSize: '2.8rem',
  fontWeight: 800,
